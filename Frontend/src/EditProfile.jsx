...
function EditProfile() {
   ...
   const [activeTab, setActiveTab] = useState("presets"); // "presets" or "upload"
       ...
       if (session?.user?.id) {
           const fetchProfile = async () => {
               const { data } = await supabase
                   .from('profiles')
                   .select('username, department, level, avatar_seed, avatar_url')
               ...
               if (data) {
                   ...
                   setCustomAvatarUrl(data.avatar_url || null);
               }
   ...
   const currentAvatarDisplay = customAvatarUrl || getAvatarUrl(currentSeed);
   ...
   const handleSaveAvatar = async () => {
       ...
       if (result.success) {
           ...
           setCustomAvatarUrl(null); // clear custom photo if switching to preset
   ...
   const handlePhotoUpload = async (e) => {
       ...
       try {
           ...
           const fileName = `avatars/${session.user.id}_${Date.now()}.${fileExt}`;
           ...
           const { error: updateError } = await supabase
               .from('profiles')
               .update({ avatar_url: publicUrl, avatar_seed: null })
               .eq('id', session.user.id);
       ...
       <>
           ...
           <div className="edit-profile-container">
                           ...
                           {activeTab === 'presets' && (
                               <>
                                   ...
                                   <div className="avatar-modal-actions">
                                       <button onClick={handleSaveAvatar} disabled={savingAvatar} className="save-avatar-btn">
                                           {savingAvatar ? "Saving..." : "Save Avatar"}
                                       </button>
                                       <button
                                           onClick={() => { setSelectedSeed(currentSeed); setShowAvatarPicker(false); }}
                           ...
                           {activeTab === 'upload' && (
                               <div className="photo-upload-section">
                                   <div className="photo-upload-preview">
                                       <img
                                           src={customAvatarUrl || currentAvatarDisplay}
                                           alt="Current avatar"
                                           className="photo-preview-img"
                                       />
                                   </div>
                                   <p className="photo-upload-hint">Upload a photo (max 2MB, JPG/PNG)</p>
                                   <input
                                       ref={fileInputRef}
                                       type="file"
                                       accept="image/*"
               ...
               <form onSubmit={handleSaveProfile}>
                   ...
                   <button className="revert-changes" type="button" onClick={() => navigate(-1)}>
                       Cancel & Revert
                   </button>
