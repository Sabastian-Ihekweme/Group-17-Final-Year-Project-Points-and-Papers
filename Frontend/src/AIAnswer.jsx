...
// Resource types for which AI features are disabled
const AI_DISABLED_TYPES = ['final year project'];
...
function AIAnswer() {
    ...
    const isAIDisabled = resource && AI_DISABLED_TYPES.includes(resource.resource_type);
    ...
    const handleGenerate = async (e) => {
        ...
        // Guard: should not be reachable via UI but protect programmatic calls too
        if (isAIDisabled) {
            setError("AI generated content is not available for final year projects.");
            return;
        }
        ...
        try {
            ...
            // Fall back to the resource row itself if resource_files is empty
    ...
    return (
        <>
            ...
            <div className="generate-ai-answer-container">
                ...
                {/* ── Final year project block ── */}
                {isAIDisabled ? (
                    <div className="ai-disabled-notice">
                        <span className="ai-disabled-icon">🚫</span>
                        <p>AI generated content is not available for final year projects.</p>
                    </div>
                ) : (
                    <>
